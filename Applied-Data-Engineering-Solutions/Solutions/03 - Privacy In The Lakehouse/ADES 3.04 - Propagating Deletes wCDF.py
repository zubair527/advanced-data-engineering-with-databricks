...
# MAGIC 
# MAGIC The architect for the current pipeline has implemented the `delete_requests` table to track users' requests to be forgotten. Note that it is possible to process delete requests alongside inserts and updates to existing data.
# MAGIC 
...
# MAGIC %md
# MAGIC We'll define a view against this table to capture those delete requests that have yet to be processed.
# MAGIC 
# MAGIC **NOTE**: Depending on your internal data retention policy, you may choose to add an additional filter and only delete those records older than a certain amount of time.

# COMMAND ----------

# MAGIC %sql
# MAGIC CREATE OR REPLACE VIEW new_requests
# MAGIC AS (SELECT alt_id FROM delete_requests WHERE status = 'requested')
...
# MAGIC 
# MAGIC We'll set a message that will be used for all subsequent operations in our notebook.
...
# MAGIC %md
# MAGIC ## Incrementally Processing Deletes
# MAGIC 
# MAGIC The `user_lookup` table was created with Change Data Feed enabled (this can be confirmed by looking at the table properties).

# COMMAND ----------

# MAGIC %sql
# MAGIC DESCRIBE EXTENDED user_lookup
...
# MAGIC %md
# MAGIC Recall that this workload is being driven by incremental changes to the `user_lookup` table. Executing the following cell will propagate deletes to a single table to multiple tables throughout the lakehouse.
