...
# MAGIC 
# MAGIC Because `foreachBatch` allows for arbitrary writes to multiple tables from the same stream, processing the delete requests to a second table can happen in this same logic.
...
# MAGIC %md
# MAGIC Now we can apply this function to our data. Here, we'll run a trigger once batch to process all records.
...
# MAGIC %md
# MAGIC Assuming some requests to be forgotten have been made, there should be records in the `delete_requests` table.
