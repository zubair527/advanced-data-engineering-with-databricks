...
# MAGIC 
# MAGIC The following CTAS statement creates a simple, unpartitioned external Delta Lake table.
...
# MAGIC 1. Avoid unnecessary storage costs (note significant differences in bytes for [numeric types](https://spark.apache.org/docs/latest/sql-ref-datatypes.html))
# MAGIC 
...
# MAGIC 
# MAGIC Users can [manually specify relational entities for which statistics should be calculated with `ANALYZE`](https://docs.databricks.com/spark/latest/spark-sql/language-manual/sql-ref-syntax-aux-analyze-table.html). While analyzing a table or a subset of columns for a table is not equivalent to indexing, it can allow the query optimizer to select more efficient plans for operations such as joins.
# MAGIC 
...
# MAGIC 
# MAGIC Z-order indexing is a technique to colocate related information in the same set of files. This co-locality is automatically used by Databricks data-skipping algorithms to dramatically reduce the amount of data that needs to be read.
# MAGIC 
